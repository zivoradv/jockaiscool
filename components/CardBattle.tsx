...
import type { Card, CardEffect, CardBattleRoom, CardBattleHand } from "@/types/game";

// ── Deck definition ────────────────────────────────────────────────────────
interface CardTemplate { name: string; attack: number; effect: CardEffect; emoji: string; color: string; description: string; count: number; }

const TEMPLATES: CardTemplate[] = [
  { name:"Strike",      attack:3, effect:"none",   emoji:"⚔️",  color:"#5e81f4", description:"Deal 3 damage.",                    count:5 },
  { name:"Heavy Blow",  attack:5, effect:"none",   emoji:"🔨",  color:"#7c3aed", description:"Deal 5 damage.",                    count:3 },
  { name:"Quick Jab",   attack:1, effect:"none",   emoji:"👊",  color:"#94a3b8", description:"Deal 1 damage.",                    count:4 },
  { name:"Power Slash", attack:6, effect:"none",   emoji:"🗡️",  color:"#c026d3", description:"Deal 6 damage.",                    count:2 },
  { name:"Fireball",    attack:8, effect:"none",   emoji:"🔥",  color:"#ff6b35", description:"Deal 8 damage. Risky!",              count:1 },
  { name:"Heal",        attack:0, effect:"heal",   emoji:"💚",  color:"#22c55e", description:"Restore 5 HP (max 20).",             count:4 },
  { name:"Shield",      attack:0, effect:"shield", emoji:"🛡️",  color:"#38bdf8", description:"Block the next attack completely.", count:3 },
  { name:"Poison Dart", attack:2, effect:"poison", emoji:"☠️",  color:"#84cc16", description:"2 damage + 2 poison each turn.",    count:3 },
  { name:"Drain",       attack:3, effect:"drain",  emoji:"🩸",  color:"#fb7185", description:"Deal 3 damage, heal yourself 2.",  count:3 },
  { name:"Double Hit",  attack:2, effect:"double", emoji:"⚡",  color:"#ffd166", description:"Deal 2 damage twice (4 total).",   count:2 },
];
...
function buildDeck(): Card[] {
  ...
  TEMPLATES.forEach(t => {
    for (let i = 0; i < t.count; i++) {
      cards.push({ id: Math.random().toString(36).substr(2,8), name:t.name, attack:t.attack, effect:t.effect, emoji:t.emoji, color:t.color, description:t.description });
    }
  });
  // shuffle
...
export default function CardBattle({ pid, onBack }: Props) {
  const [screen, setScreen]   = useState<"landing"|"lobby"|"playing"|"gameover">("landing");
  ...
  const createRoom = async () => {
    ...
    const r: CardBattleRoom = {
      ...
      status: "lobby", turn: pid,
      hp: { [pid]: MAX_HP },
      shields: { [pid]: false },
      poison: { [pid]: 0 },
      lastPlayed: null, winner: null,
      handSizes: { [pid]: HAND_SIZE },
      totalCards: deck.length,
    };
    await set(roomRef(c), r);
    await set(handRef(c, pid), { hand, deck });
    setCode(c); setRoom(r); setMyHand(hand); setMyDeck(deck);
    setScreen("lobby"); setLoading(false);
  };
  ...
  const joinRoom = async () => {
    ...
    await set(handRef(c, pid), { hand, deck });
    setCode(c); setRoom(updated); setMyHand(hand); setMyDeck(deck);
    setScreen("playing"); setLoading(false);
  };
  ...
  const playCard = async (card: Card) => {
    ...
    setMyHand(newHand); setMyDeck(newDeck);
    await set(handRef(c, pid), { hand: newHand, deck: newDeck });

    // 4. Check win
    const winner = newHp[other.id] <= 0 ? pid : newHp[pid] <= 0 ? other.id : null;

    await set(roomRef(c), {
      ...r, hp: newHp, shields: newShields, poison: newPoison,
      lastPlayed: { pid, card },
      handSizes: { ...r.handSizes, [pid]: newHand.length },
      turn: other.id,
      winner, status: winner ? "gameover" : "playing",
    });
  ...
  const playAgain = async () => {
    ...
    const hand = deck.splice(0, HAND_SIZE);
    await set(handRef(c, pid), { hand, deck });
    setMyHand(hand); setMyDeck(deck);
    if (r.host === pid) {
      const other = r.players.find(p => p.id !== pid);
      const hp: Record<string,number> = {};
      const shields: Record<string,boolean> = {};
      const poison: Record<string,number> = {};
      r.players.forEach(p => { hp[p.id] = MAX_HP; shields[p.id] = false; poison[p.id] = 0; });
      await set(roomRef(c), {
        ...r, hp, shields, poison, status: "playing",
        turn: r.host, lastPlayed: null, winner: null,
        handSizes: { ...r.handSizes, [pid]: HAND_SIZE },
      });
    }
    setScreen("playing");
  ...
  return (
    <>
      ...
      <div className="gn-app">
        ...
        {screen === "landing" && (
          <GameLanding gameName="Card Battle" gameEmoji="⚔️"
          ...
          <div className="gn-content">
            ...
            <div className="cb-hp-section">
              ...
              <div className="cb-player-row">
                <div className="cb-avatar cb-avatar-them">{them?.name?.[0]?.toUpperCase() ?? "?"}</div>
                <div className="cb-player-info">
                  <div className="cb-player-name">
                    {them?.name ?? "Partner"}
                    {room?.shields[them?.id ?? ""] && <span className="cb-shield-badge">🛡️</span>}
                    {(room?.poison[them?.id ?? ""] ?? 0) > 0 && <span className="cb-poison-badge">☠️ ×{room?.poison[them?.id ?? ""]}</span>}
                  </div>
                  <HpBar hp={themHp} maxHp={MAX_HP} color="#5ee7ff" />
                  <div className="cb-hp-text">{themHp} / {MAX_HP} HP · {room?.handSizes[them?.id ?? ""] ?? 0} cards</div>
                </div>
              </div>
                ...
                {room?.lastPlayed ? (
                  <div className="cb-last-played">
                    <div className="cb-last-label">
                      {room.lastPlayed.pid === pid ? "You played" : `${them?.name} played`}
                    </div>
                    <div className="cb-mini-card" style={{ borderColor: room.lastPlayed.card.color, boxShadow: `0 0 18px ${room.lastPlayed.card.color}40` }}>
                      <span className="cb-mini-emoji">{room.lastPlayed.card.emoji}</span>
                      <span className="cb-mini-name">{room.lastPlayed.card.name}</span>
                    </div>
                  </div>
                ) : (
                  <div className="cb-arena-empty">⚔️ Game Start!</div>
                )}
                <div className={`cb-turn-badge ${isMyTurn ? "cb-your-turn" : "cb-their-turn"}`}>
              ...
              <div className="cb-player-row cb-player-row-me">
                <div className="cb-avatar cb-avatar-me">{me?.name?.[0]?.toUpperCase() ?? "?"}</div>
                <div className="cb-player-info">
                  <div className="cb-player-name">
                    You
                    {room?.shields[pid] && <span className="cb-shield-badge">🛡️</span>}
                    {(room?.poison[pid] ?? 0) > 0 && <span className={`cb-poison-badge ${poisonAnim ? "cb-poison-anim" : ""}`}>☠️ ×{room?.poison[pid]}</span>}
                  </div>
                  <HpBar hp={myHp} maxHp={MAX_HP} color="#ff6eb4" />
                  <div className="cb-hp-text">{myHp} / {MAX_HP} HP · {myHand.length} cards · {myDeck.length} in deck</div>
                </div>
              </div>
            </div>
            ...
            {lastLog.length > 0 && (
              <div className="cb-log">
                {lastLog.map((l, i) => <div key={i}>{l}</div>)}
              </div>
            )}

            {/* Hand */}
            <div className="cb-hand-label">
              {isMyTurn ? "Your Hand — Pick a card to play" : `Waiting for ${them?.name ?? "partner"}…`}
            </div>
            <div className="cb-hand">
              {myHand.map(card => (
                <button
                  key={card.id}
                  className={`cb-card ${isMyTurn ? "cb-card-active" : "cb-card-disabled"} ${playingCard === card.id ? "cb-card-playing" : ""}`}
                ...
                >
                  ...
                  {card.attack > 0 && <span className="cb-card-atk">⚔️ {card.effect === "double" ? card.attack * 2 : card.attack}</span>}
        ...
        {screen === "gameover" && (
          <div className="gn-content">
            <div className="gn-card" style={{ alignItems: "center" }}>
              ...
              <div className="gn-result-title" style={{ color: iWon ? "var(--gold)" : "var(--red)" }}>
                {iWon ? "Victory!" : "Defeated!"}
              </div>
              <p className="gn-small gn-muted gn-center" style={{ marginBottom: 16 }}>
                {iWon ? `You destroyed ${them?.name}!` : `${room?.players.find(p=>p.id===room?.winner)?.name} wins this time!`}
              </p>
              {/* Final HP */}
              <div className="cb-final-hp">
                <div className="cb-final-hp-card">
                  <div className="cb-final-hp-name">You</div>
                  <div className="cb-final-hp-val" style={{ color: "var(--pink)" }}>{myHp} HP</div>
                </div>
                <div className="cb-final-hp-card">
                  <div className="cb-final-hp-name">{them?.name}</div>
                  <div className="cb-final-hp-val" style={{ color: "var(--cyan)" }}>{themHp} HP</div>
                </div>
              </div>
              <button className="gn-btn gn-btn-primary" style={{ width: "100%" }} onClick={playAgain}>🔁 Play Again</button>
              <button className="gn-btn gn-btn-ghost" onClick={onBack}>← Back to Hub</button>
            </div>
          </div>
        )}
      </div>
...
function HpBar({ hp, maxHp, color }: { hp: number; maxHp: number; color: string }) {
  ...
  return (
    <div className="cb-hp-bar-bg">
      <div className="cb-hp-bar-fill" style={{ width: `${pct}%`, background: barColor, boxShadow: `0 0 10px ${barColor}80` }} />
    </div>
  );
}
